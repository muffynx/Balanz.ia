    ...
    <div>
      ...
      <div>
        ...
        >
          ...
          <svg
            className="w-5 h-5 text-gray-400 group-hover:text-[#299D91] transition-colors"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 overflow-y-auto pb-4 flex-grow">
              {filteredCategories.length > 0 ? (
                filteredCategories.map((cat) => (
                  <div
                    key={cat._id}
                    className={`bg-white/80 backdrop-blur-sm border-2 rounded-xl p-3 flex-shrink-0 transition-all duration-300 shadow-lg hover:shadow-xl flex flex-col items-center justify-between space-y-2 cursor-pointer h-28 group ${
                      isManaging
                        ? "border-dashed border-2 border-orange-300 hover:bg-red-50/50 hover:border-red-300"
                        : ""
                    } ${
                  ...
                  >
                    <div className="w-full text-center text-gray-800 font-medium flex flex-col items-center flex-grow justify-center">
                      <div
                        className={`p-2 rounded-xl mb-2 transition-all duration-200 ${
                          formData.category === cat._id && !isManaging
                      ...
                      >
                        <span
                          className={`text-2xl ${
                            formData.category === cat._id && !isManaging
                              ? "text-white"
                              : "text-gray-600 group-hover:text-[#299D91]"
                          }`}
                        >
                          {cat.icon}
                        </span>
                      ...
                      <span
                        className={`text-xs font-bold truncate ${
                          formData.category === cat._id && !isManaging
                            ? "text-[#299D91]"
                            : "text-gray-700 group-hover:text-[#299D91]"
                        }`}
                      >
                        {cat.name}
                      </span>
